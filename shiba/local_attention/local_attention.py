...
def pad_to_multiple(tensor, multiple, dim=-1, value=0):
    # during tracing, seqlen is returned as tensor, not int
    seqlen = tensor.shape[dim]
    div = torch.div(seqlen, multiple, rounding_mode="floor")
    is_integer = torch.eq((seqlen - div * multiple), 0)
    if is_integer:
        return tensor
...
class LocalAttention(nn.Module):
    ...
    def forward(self, q, k, v, input_mask = None):
        ...
        windows = torch.div(t, window_size, rounding_mode='trunc')
        ...
        if input_mask is not None:
            h = torch.div(b, input_mask.shape[0], rounding_mode='trunc')
            if self.autopad:
                input_mask = pad_to_multiple(input_mask, window_size, dim=-1, value=False)
            input_mask = input_mask.reshape(-1, windows, window_size)
            mq = input_mask.clone().detach()
            mk = input_mask.clone().detach()
            mk = look_around(mk, pad_value=False, **look_around_kwargs)
            # onnx opset ver 13 doesn't support bool*bool matrix ops
            mqu = mq[:, :, :, None].type(torch.float16)
            mku = mk[:, :, None, :].type(torch.float16)
            mask = torch.mul(mqu, mku).type(torch.bool)
